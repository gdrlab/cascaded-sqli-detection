...
| **Description:**
| This module provides methods related to Bag of Characters (BoC) Approach which is an NLP algorithm to extract features from the datasets. It converts text from the datasets into matrices which can then be processed by the machine learning algorithms.
...
# Processed training dataset using BoC approach
X_train = None
y_train = None

# Processed testing dataset using BoC approach
X_test = None
y_test = None
...
# Bag of Characters (BoC) resolution is limited to characters in the text instead of words.
...
def extract_features_training_dataset():
	"""
	It extracts features from training dataset using Bag of Characters (BoC) Approach.  
	"""
	...
	cv = CountVectorizer(lowercase=True,stop_words='english',ngram_range = (1,int(package.configurations.NGRAM_RANGE_BOC)),tokenizer = token.tokenize, analyzer='char')
...
def extract_features_testing_dataset(cv):
	"""
	It extracts features from testing dataset using Bag of Characters (BoC) Approach.  
	"""
...
def extract_features():
	"""
	It extracts features from training and testing datasets using Bag of Characters (BoC) Approach.  
	"""
	
	logging.info('---------------------------------------------------------------------------------')
	logging.info('Features extraction using Bag of Characters (BoC) Approach')
	logging.info('---------------------------------------------------------------------------------')
...
def analyze_naive_bayes():
	...
	if (X_train is not None) or (y_train is not None) or (X_test is not None) or (y_test is not None):
		...
		logging.info('\r---------------------------------------------------------------------------------')
		logging.info('\r Multinomial Naive Bayes Algorithm using Bag of Characters (BoC)')
		logging.info('\r---------------------------------------------------------------------------------')
...
def analyze_xgboost():
	...
	if (X_train is not None) or (y_train is not None) or (X_test is not None) or (y_test is not None):
		...
		logging.info('\r---------------------------------------------------------------------------------')
		logging.info('\r XGBoost Algorithm using Bag of Characters (BoC)')
		logging.info('\r---------------------------------------------------------------------------------')
...
def analyze_svm():
	...
	if (X_train is not None) or (y_train is not None) or (X_test is not None) or (y_test is not None):
		...
		logging.info('\r---------------------------------------------------------------------------------')
		logging.info('\r SVM Algorithm using Bag of Characters (BoC)')
		logging.info('\r---------------------------------------------------------------------------------')
